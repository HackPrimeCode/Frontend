...
export default function LandingPage() {
  ...
  return (
    <div className="w-full min-h-[calc(100vh-3.75rem)] bg-background">
      <section className="w-full px-36 pt-16 pb-17 flex flex-col items-center">
        <div className="w-full max-w-4xl flex flex-col items-center text-center">
          <h1 className="text-5xl md:text-[5.25rem] text-white mb-4">
            Hack<span className="text-red">Prime</span>Code
          </h1>
          <p className="text-xl md:text-3xl text-white mb-9">
            Code. Compete. Conquer.
          </p>
          <p className="text-text-accent max-w-2xl">
      ...
      <section className="w-full px-6 pb-12">
        <div className="w-full max-w-6xl mx-auto flex flex-col space-y-8">
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
            ...
            <div className="flex gap-1 bg-input-background border border-border rounded-lg p-1">
              {filters.map((filter) => (
                <Button
                  key={filter.id}
                  onClick={() => setActiveFilter(filter.id)}
                  className={`px-4 py-2 text-xs rounded-md transition-all cursor-pointer ${
      ...
      <section className="w-full px-6 pb-8.5 border-b border-border">
        <div className="w-full max-w-6xl mx-auto bg-card-background border border-border rounded-lg px-9 py-5 flex flex-col md:flex-row items-center justify-between gap-4">
          <div>
            <h3 className="text-white text-lg mb-2">Готов принять участие?</h3>
          ...
          <Button
            onClick={() => navigate("/hub", { replace: true })}
            className="h-12 px-6 bg-red text-white text-sm rounded-lg flex items-center gap-2 hover:bg-red/90 transition-colors cursor-pointer whitespace-nowrap"
          >
            Все мероприятия
            <img
              src="/arrow-white-icon.svg"
