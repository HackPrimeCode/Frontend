...
interface JuryRegisterFormProps {
  token: string;
  email: string;
}

const juryRegisterFormSchema = z
  .object({
    token: z.string(),
...
type JuryRegisterFormValues = z.infer<typeof juryRegisterFormSchema>;

export default function JuryRegisterForm({
  token,
  email,
}: JuryRegisterFormProps) {
  const form = useForm<JuryRegisterFormValues>({
    resolver: zodResolver(juryRegisterFormSchema),
    defaultValues: {
      token: token || "",
      fullName: "",
      email: email || "",
  ...
  function onSubmit(data: JuryRegisterFormValues) {
    console.log("Регистрация жюри:", data);
  }
