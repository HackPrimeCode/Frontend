...
export default function TaskTab() {
  ...
  const { data: hackathonDetails, isLoading: isHackathonLoading } =
    useGetHackathonDetailsWithTaskQuery(Number(hackathonId)!, {
      skip: !hackathonId,
    });

  const { days, hours, minutes, seconds, isExpired } = useCountdown(
    hackathonDetails?.end_date,
  );
  ...
  if (isLoading || isHackathonLoading) {
    return (
      <div className="text-white text-sm p-6 animate-pulse">
        Загрузка задания хакатона...
      </div>
    );
  }
  ...
  if (!team || !hackathonId) {
    return (
      <div className="text-text-accent text-sm p-6 text-center border border-dashed border-border rounded-xl bg-card-background/20 max-w-2xl mx-auto">
        Вы не зарегистрированы на активный хакатон в составе команды.
        Спецификация и файлы задания будут доступны после вступления в команду
        проекта.
      </div>
    );
  }
  ...
  if (!hackathonDetails) {
    return (
      <div className="text-text-accent text-sm p-6 text-center border border-dashed border-border rounded-xl bg-card-background/20 max-w-2xl mx-auto">
        Не удалось загрузить данные технического задания хакатона. Пожалуйста,
        обновите страницу.
      </div>
    );
  }
      ...
      <div className="px-4.5 py-2">
            ...
            <div className="flex flex-col justify-between gap-1">
              ...
              <div className="flex flex-wrap items-center gap-x-4 gap-y-2.5 text-text-accent">
                ...
                <div className="flex items-center gap-1.5 text-xs whitespace-nowrap">
                  ...
                  <div>
                    <p>
                      {formatDate(hackathonDetails?.start_date)} —{" "}
                      {formatDate(hackathonDetails?.end_date)}
                    </p>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-1.5 text-xs whitespace-nowrap">
                  ...
                  <span>
                    {hackathonDetails?.current_participants} /{" "}
                    {hackathonDetails?.max_participants} участников
                  </span>
                </div>
              </div>
            </div>
        ...
        <div className="flex bg-card-background border border-border p-1 rounded-lg w-full sm:w-fit text-white text-xs mb-4">
          ...
          >
            ...
            <span className="text-xs px-1.5 py-0.5 rounded-md text-white bg-white/20">
              {hackathonDetails?.files.length ?? 0}
            </span>
          </button>
        </div>
        ...
        <div className="w-full">
          {activeContentTab === "spec" ? (
            <TaskSpecTab hackathon={hackathonDetails} />
          ) : (
            <TaskFilesTab files={hackathonDetails?.files || []} />
          )}
        </div>
      </div>
    </div>
